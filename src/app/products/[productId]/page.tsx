...
export async function generateMetadata({
  ...
  try {
    ...
    if (!resp.ok) {
      return {
        title: "Product Not Found | ShopMart",
    ...
    return {
      title: "Product | ShopMart",
      description: "Shop quality products at ShopMart",
    };
  }
}
...
export default async function ProductDetails({
  ...
  try {
    ...
    if (!resp.ok) {
      console.error(
        `Failed to fetch product: ${resp.status} ${resp.statusText}`
      );
      notFound();
    }
    ...
    // Fetch related products from the same category with error handling
    ...
    try {
      ...
      if (relatedResp.ok) {
        ...
        relatedProducts = (relatedData.data as ProductI[]).filter(
          (p) => p._id !== product._id
